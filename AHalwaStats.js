...
const rotten = document.getElementById("kharab");
    ...
    fetch("fetch_halwa_stats.php")
        ...
        .then(data => {
            data.forEach(expense => {
                totalLoss += Number(expense.weight_loss);
                totalKharab += Number(expense.rotten);
            });

            animateCounter(loss, 0, totalLoss, 1000, "", " Kgs");
            animateCounter(rotten, 0, totalKharab, 1000, "", " Kgs");
        })
        .catch(error => {
...
loss.textContent = `${totalLoss} Kgs`;
rotten.textContent = `${totalKharab} Kgs`;
...
function renderTransactions(data) {
  ...
  data.forEach(stat => {
    ...
    row.innerHTML = `
      ...
      <td>${stat.weight_loss}</td>
      <td>${stat.rotten}</td>
      <td>${stat.kgs_delivered}</td>
      <td>${stat.decription}</td>
      <td>
      ...
      <td colspan="25">
        <div class="next">
          ...
          <div class="align"><label>Peti Loss (Pcs): </label>
            <input type="text" id="weight_loss-${stat.week}" value="${stat.weight_loss}" />
          </div>

          <div class="align"><label>Kharab (Pcs): </label>
            <input type="text" id="rotten-${stat.week}" value="${stat.rotten}" />
          </div>
...
document.getElementById("addExpense").addEventListener("submit", function(event) {
    ...
    .then(data => {
        if (data.status === "success") {
            ...
            this.reset(); 
            fetchTransactions();
            loadStats(); // Reload stats summary
        } else {
            alert(data.message || "Failed to add stat.");
        }
    })
    .catch(error => {
        console.error("Error:", error);
        alert("Error adding stat. Please try again.");
    });
});