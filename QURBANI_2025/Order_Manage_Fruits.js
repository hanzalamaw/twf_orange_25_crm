...
let displayType = 0; //0 -> Qurbani & 1 -> FruitS
const qurbani = document.getElementById("0");
const fruits = document.getElementById("1");

function setDisplayType(newType){
  displayType = newType;
  switch(newType){
    case 0: 
      qurbani.classList.remove("summary-card");
      qurbani.classList.add("selected");

      fruits.classList.remove("selected");
      fruits.classList.add("summary-card");
      break;

    case 1: 
      fruits.classList.remove("summary-card");
      fruits.classList.add("selected");

      qurbani.classList.remove("selected");
      qurbani.classList.add("summary-card");
      break;
  }
}
...
function renderTransactions(data) {
  ...
  data.forEach(order => {
    ...
    dropdownRow.innerHTML = `
      <td colspan="25">
        ...
        <div class="align"><label>Quantity: </label> <input type="text" id="day-${order.order_id}" value="${order.day}" /></div>
        <div class="align"><label>Order Type: </label> <input type="text" id="order_type-${order.order_id}" value="${order.order_type}" /></div>
        <div class="align"><label>Weight: </label> <input type="text" id="shareholder_name-${order.order_id}" value="${order.shareholder_name}" /></div>
  ...
  document.getElementById("1").addEventListener("click",  () => {
    ...
    const validTypes = ["Mango - Chaunsa", "Mango - Sindhri", "Mango - Anwar Ratol"];

    const filteredData = data.filter(order => validTypes.includes(order.order_type));
...
function filterTransactions() {
  ...
  const cowMin = parseInt(document.getElementById("cowMin").value) || null;
  const cowMax = parseInt(document.getElementById("cowMax").value) || null;
  ...
  const filtered = allOrders.filter(order => {
      ...
      const cow = parseInt(order.cow_number);
      const cowMatch =
          (cowMin === null && cowMax === null) ||
          (cowMin !== null && cowMax !== null && cow >= cowMin && cow <= cowMax) ||
          (cowMin !== null && cowMax === null && cow >= cowMin) ||
          (cowMin === null && cowMax !== null && cow <= cowMax);

      return bookingMatch && phoneMatch && statusMatch && cowMatch;
  });
...
function exportToCSV() {
  ...
  const headers = [
      "Customer ID", "Order ID", "Cow #", "Hissa #", "Booking Name", "Shareholder Name", "Phone Number", "Alt. Phone",
